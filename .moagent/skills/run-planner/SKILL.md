...
```bash
python .moagent/skills/run-planner/scripts/intent_clarifier.py --question "<用户问题>" --capability "<capabilityId>"
```

也可把 JSON 对象、JSON 文件路径或标准输入交给 `--input`：

```bash
python .moagent/skills/run-planner/scripts/intent_clarifier.py \
