...
把 Query Rewrite 尚未确定的股票名称、简称、拼音或代码解析成标准证券代码和东方财富 `secid`。优先复用 `.data-agent/finance-query-rewrite.json.resolvedSymbols`，不得重复解析已经确认的标的。
...
```bash
python .moagent/skills/quant-symbol-resolver/scripts/rank_candidates.py \
