...
它用于比较标的、复盘策略和检查数据质量，不执行自动交易，也不提供买卖建议。通过浏览器访问，核心服务由 Next.js、Python 市场数据服务、PostgreSQL/TimescaleDB 和 Redis 组成。
...
“可用”表示能力已接入当前任务链，不保证任意问题都能一次生成成功；真实模型任务还要看数据、验证和预览结果。
...
一个生成项目会留下页面源码、任务与金融 run plan、原始行情或回测实验文件、供页面消费的最终数据、来源/质量证据，以及 build、产物和视觉验证报告。平台还会持久化项目、请求、AgentRun、Mission 与验收状态。预览出错时，可以据此定位规划、数据、工具、构建或交付验收阶段。

## 项目设计亮点

### 规划和证券身份分开

Query Rewrite 使用当前所选模型生成 schema v4 语义合同，提取研究意图、时间窗口与交付类型；证券 Resolver 再把名称或代码核对为标准标的。模型不可用或关键字段缺乏原文证据时停止后续取数。

### 回测保存计算输入

市场数据服务记录复权方式、区间、参数、费率、原始 K 线与计算结果摘要。工作空间保存原始实验文件和最终看板引用；离线 replay 可检查输入和结果哈希是否一致。“可复算”仍不代表具备历史 point-in-time 数据或实盘执行条件。

### 交付以验收回执为准

PI Agent 根据数据和 Skills 写入工作空间后，平台继续检查构建、数据证据、产物合同和预览 HTTP 状态。Mission 接受证据回执后，任务才进入完成态；失败会留下阶段和验证报告。工作空间写入后要求明确提交结果。

### 长任务和生成代码各有边界

PostgreSQL generation job、事务 outbox、Worker registry、配额、lease/fencing 和项目单写约束负责派发与恢复；受信副作用工具支持审批。生成项目的 build/preview 在 Linux namespace 沙箱内执行，默认不注入平台密钥，预览只获得受限的行情通道。

### 评测保留失败过程

评测将事实/证据、任务完成、产物、视觉、运行可靠性与安全边界分开记录；合同用例与真实模型 E2E 分开运行。重复评估固定数据与配置，并保留首轮通过、修复尝试、失败阶段和耗时。可选组件尚未全部在本机完成生产联调，不能把诊断通过写成完整服务验收。
...
数据与证据落在生成工作空间，数据库保存索引、状态与摘要。构建缓存可以重建，原始实验记录和 Mission 验收证据需要保留。
...
真实 E2E 使用 `npm run benchmark:quant:e2e -- --model deepseek-v4-flash`，会消耗 API 额度；应按固定用例保存模型、数据快照、输入、失败阶段和预览状态。2026-09-16 在 WSL2 对代码提交 `ed9917a` 的本地检查结果：**1348 个单测通过（27 跳过）、146 个后端测试通过（4 跳过），类型检查与构建通过**。这是一轮本机结果，不代表长期稳定性或生产可用性。
...
核心研究链路可以在 WSL2 中使用 DeepSeek 官方直连运行。真实模型任务仍会受到模型响应、行情供应商和交易日数据新鲜度影响；行业/板块分析与策略研究仍是规划能力。Memory、ModelPort 本地模型和 Loki 需要各自的服务与配置，关闭时不妨碍官方直连的核心任务。后续优先固定研究用例与数据快照，持续记录模型失败阶段和交易日数据覆盖情况。

判断一次看板任务是否完成，要核对证券与数据证据、通过的构建、Mission accepted receipt 和预览 HTTP 状态。缺少其中一项时，先查任务阶段与验证报告，不把模型最后一条回复当成交付结果。
