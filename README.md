...
QuantPilot 是面向量化投研与金融数据分析场景的 AI 工作台。当前阶段聚焦应用生成、项目预览、Agent 调用、模型接入、金融数据后端、Skills 管理和可验证看板生成，并将默认模型运行方式配置为 Claude Code 运行时直连 MiniMax 的 Anthropic-compatible 接口。
...
Agent 使用的项目级 skills 统一放在 `.claude/skills/`，生成项目时会被复制到对应项目目录中，方便 Agent 在任务执行过程中读取和调用。
...
本地可以通过 `http://localhost:3000/skills` 管理 skills。当前页面支持查看健康状态和版本记录、在线编辑 `SKILL.md`、发布 semver 版本、重新打包 lock，也支持拖拽 `.zip`、`.tgz` 或 `.tar.gz` 作为新版本发布。发布失败时会回滚 registry、changelog、lock 和压缩包，上传失败时会回滚源码目录。
...
每个 skill 也可以打包成独立压缩包，默认位置是 `.claude/skill-packages/<skill-id>.tgz`。生成项目时平台默认只安装 10 个核心 skill，优先解压压缩包到 `<project>/.claude/skills/`，压缩包不存在时回退复制源码目录：
...
- `quant-run-planner`：意图澄清、任务规划、run plan 生成与用户补充信息承接。
- `quant-data-registry`：查询当前可用数据源和接口能力。
- `quant-symbol-resolver`：把中文股票名、简称或代码解析成标准证券代码。
- `quant-image-extraction`：从持仓截图、表格截图和用户上传图片中提取结构化字段。
- `quant-market-data`：获取实时行情、历史 K 线、指数 ETF、批量行情和行情兜底信息。
- `quant-fundamentals`：获取财务报表、财务指标、公告事件和估值相关信息。
- `quant-indicators`：计算技术指标、收益、回撤、波动率、相关性和趋势模板。
- `quant-backtest`：执行策略回测，输出净值、回撤、交易明细、胜率和参数限制。
- `quant-data-quality`：在取数后生成 `evidence/sources.json` 与 `evidence/data_quality.json`，记录来源、时间戳、缺失字段、警告和限制。
- `quant-visualization-html`：基于已获取的数据生成可视化 HTML/Next.js 看板，禁止外部 CDN、mock 数据和明文密钥。

旧的细粒度 skill 名称保留为 legacy alias，仅用于兼容历史 workspace，不再作为新能力扩张方向。
...
Agent 执行完成后，QuantPilot 会自动对生成项目执行一轮验收，并把摘要写回聊天记录。验证报告保存在生成项目的 `.quantpilot/validation.json`，同时会在 `.quantpilot/events.jsonl` 记录 `validation_started` 和 `validation_completed` 事件。验证失败时，平台会额外生成 `.quantpilot/validation-repair-plan.json`，把失败项拆成可执行修复步骤，并自动触发一次 Agent 修复。
...
- 生成产物策略：页面和配置不得引用外部 CDN、远程脚本、远程样式、远程字体或浏览器直连外部 API，不得留下 mock/static/sample 数据，不得写入 token、api key、cookie 或 authorization。
...
接口返回中 `data` 是验证报告，`repairPlan` 是当前失败项对应的结构化修复计划；验证通过后修复计划会被自动清理。
...
截图会写入 `tmp/visual-checks/`。`check:validation-repair` 用于确认验证失败时能生成结构化修复计划和 Agent 修复提示词；`check:generated-artifacts` 用于确认生成项目不会退回外部 CDN、mock 数据、明文密钥或非标准数据绑定；`check:benchmark-coverage` 用于确认量化 benchmark 没有漏掉核心能力、资产类型、图片输入、澄清承接、数据源降级和视觉冒烟。检查项目看板时请使用 `http://localhost:3000`，这也是用户实际访问地址；不要混用 `127.0.0.1:3000` 做截图判断，避免 dev server 的客户端接管表现不一致。

## 量化回归评测

固定问句和预期产物定义位于 `benchmarks/quantpilot/cases.json`。回归脚本会创建临时项目，执行 run plan、平台预取、evidence 生成、默认看板模板和自动验证，不直接依赖模型输出。报告会额外写入 `coverage` 覆盖矩阵和每个项目的 `eventAudit`，用于确认能力覆盖、输入形态、数据源、验证规则和 `.quantpilot/events.jsonl` 过程事件闭环。
...
评测报告写入 `tmp/quantpilot-benchmark-reports/`，该目录不进入 Git。当前内置 12 个用例覆盖：

- 贵州茅台基本面看板。
- 沪深300指数技术趋势。
- 沪深300ETF技术趋势。
- 510300 均线突破回测。
- 贵州茅台、平安银行、宁德时代多标的横向选股对比。
- 杭钢股份、京沪高铁、三七互娱、中国黄金、完美世界持仓调仓风险分析。
- 缺少关键标的时进入意图澄清，不取数也不生成错误看板。
- 上传持仓截图时生成图片附件证据、`imageExtraction` 契约和持仓风险看板，并通过 Playwright 截图冒烟。
- 用户补充澄清信息后自动合并上一轮问题，继续原始 run plan。
- Codex CLI 只暴露第三方 OpenAI-compatible `gpt-5.5`，默认使用 `low` reasoning。
- 自动验证失败时生成结构化修复计划和 Agent 修复提示词。
- 东方财富数据缺口场景下保留备用信源、warning 级数据质量证据和页面限制说明。
