...
`db:init` 会先执行 `sqls/*.sql`，再运行 `prisma migrate deploy` 应用 Prisma 管理的版本化结构，并幂等补齐内置权限/配额策略以及校准历史项目的 `projects.owned` allocation。直接运行 `npm run prisma:deploy` 或兼容别名 `npm run prisma:push` 也会串联同一 bootstrap；原始 `npx prisma db push` 无法表达 migration 中的全部 CHECK/部分唯一索引，不应作为初始化或发布入口。
