...
| 使用归因 | QuantPilot PostgreSQL `external_memory_uses` | 只保存 trace/revision 不透明 ID、策略版本和内容哈希 |
...
`prepared` 只表示生成了候选 capsule，不证明 Agent 已使用。QuantPilot 仅在把 capsule 交给 MoAgent 前写入本地归因；澄清、拒绝、平台直接生成或用户临时关闭个性化时不会写“已暴露”记录。已验证完成的回复只有查到这份归因后，才显示“本轮实际使用了 N 条个人偏好”及反馈按钮。
...
| `QUANTPILOT_MEMORY_TENANT_ID` | QuantPilot 所属稳定 tenant，最长 128 字符 |
