# 运行治理中心使用与评分指南

运行治理中心负责回答一个朴素但关键的问题：这个系统现在到底靠不靠谱？

入口：

```text
http://localhost:3000/ops-platform
```

它不是为了多一个控制台，而是把工作空间、生成链路、基础环境、策略数据和日志这些分散事实集中起来。开发时看它可以少猜很多事；演示或上线前看它，可以知道问题是在环境、数据、Agent、页面还是验证链路。

## 运行治理中心看什么

| 视图 | 主要问题 |
| --- | --- |
| 运行总览 | 系统能否运行、Worker 是否消费队列、当前有几个阻断、应该先处理什么 |
| 服务治理 | 服务目录、依赖级别、端点、命令、基础环境与平台能力是否可用 |
| 工作空间 | 生成项目是否缺产物、验证是否通过、是否应进入修复或归档 |
| 生成链路 | 当前请求走到哪一步、最后失败在哪个阶段、修复是否触发 |
| 运行日志 | Loki 集中日志、本地 Next 日志、评测队列日志和运行日志 |

运行总览还会并列展示项目健康、运行健康与策略健康三层画像。这样历史工作空间失败、可选日志组件降级和策略数据过期不会被混成一个笼统的“平台不可用”。

### 研究闭环指标

运行总览默认聚合最近 7 天的产品结果，用于区分“系统正常”和“用户真正完成了投研任务”。

| 指标 | 口径 |
| --- | --- |
| 研究请求 | 创建时间位于窗口起点至本次采集时间（两端包含）的 `user_requests`，同时展示活跃项目和进行中请求；完成率为 completed / (completed + failed + cancelled) |
| Mission 验收率 | 对应请求的终态 Mission 中，通过验收的比例；completed、failed、cancelled 进入分母，等待输入、运行、验证、修复不进入分母 |
| 验收证据完整率 | 已完成 Mission 中，有匹配的 acceptance / accepted 回执的比例；同时核对回执 ID、Mission 归属和 candidateVersion，缺失或不匹配会显示数量提示 |
| 交付中位耗时 / P90 / P95 | 只统计通过验收的 Mission，从请求创建到 Mission 完成的时长；缺失、无效、负值和晚于采集时间的完成时间排除并提示。分位数使用排序后线性插值，无有效样本显示“暂无” |
| 窗口内重复研究率 | 窗口内提交至少两次请求的已识别研究者 / 窗口内全部已识别研究者；匿名请求不进入分母。这是重复使用比例，不是第 7 日留存或首个研究完成率 |
| 结构化研究报告 | 按 `reportDate` 统计完整窗口内报告数量，与请求采样独立 |

请求查询最多读取 10,001 条来检测截断，按创建时间、ID 倒序，仅聚合最近 10,000 条。超过上限时，页面明确提示请求、Mission、耗时和研究者指标不能代表完整窗口；这不是随机抽样。各比例在分母为空时显示“暂无”，不显示 0%。状态是本次读取时的状态，历史窗口不会冻结成事件快照。

数据库被显式停用时，该区域显示降级状态而不发起连接；同页的工作空间列表、生成 trace 和 Worker 观测也跳过数据库读取，避免在渲染提示前整页报错。产品指标自身查询失败时显示不可用提示，不显示一组零值。仍启用且要求可用的数据库发生故障时，保留原有失败语义。`/api/ops/platform` 在读取指标前校验观测权限，成功、授权失败和异常响应均禁止缓存，内部错误不返回数据库连接细节。

指标聚合在 [product-health.ts](../src/lib/ops/product-health.ts)，展示组件在 [OpsProductHealth.tsx](../src/app/ops-platform/OpsProductHealth.tsx)。当前尚未采集完整的首个有效研究 cohort 和任务金额账单，不能从这组指标推导首次交付耗时、7 日留存或单任务美元成本。

浏览器回归使用标准 Playwright spec，覆盖桌面与移动端的降级提示、指标刷新、接口失败后保留已有指标，以及横向溢出检查：

```bash
npx playwright install chromium
npm run build
npm run test:e2e
```

测试自动启动 `127.0.0.1:3107` 上的独立本地服务，使用空工作空间并关闭认证和外部组件，仅用于 UI 合同验收，不复用正在运行的服务。指标成功场景使用显式的合成 API 数据；它不能替代真实 Mission 或金融数据 E2E。截图与失败 trace 写入 `test-results/product-health/`，HTML 报告在 `playwright-report/`。本机已有浏览器时可通过 `QUANTPILOT_CHROMIUM_EXECUTABLE_PATH` 指定 Chromium 可执行文件；CI 安装 Playwright 配套版本。

同一套浏览器门禁还覆盖聊天文件编辑器：真实页面连接内存 API fixture，验证保存、失败重试、取消关闭保留草稿，以及移动端编辑宽度。测试拦截所有业务 API，不创建真实项目、修改工作空间文件或触发模型任务。ESLint 排除上述报告目录，仍检查 spec 本身。

如果你只想知道“页面为什么打不开”，先看基础环境。如果你想知道“某个生成工作空间为什么不可用”，先看工作空间健康。如果你想知道“为什么策略平台结果不可信”，先看策略健康和数据覆盖。

### Data Agent 执行池

运行总览的执行池不是读取某一个进程的内存计数，而是聚合三类数据库事实：

| 事实 | 数据表 | 回答的问题 |
| --- | --- | --- |
| Worker registry | `agent_worker_instances` | 有几个进程存活、运行在哪个主机/PID、单进程和全局容量是否一致 |
| 全局槽位 | `agent_worker_slots` | 当前到底占用了多少集群执行容量、哪个 Job/Worker 持有槽位、租约是否过期 |
| Generation queue | `agent_generation_jobs`、`user_requests` | pending/retry/running 数量、排队用户数、最久等待和 24 小时完成/失败量 |

`PI_AGENT_DISPATCH_MODE=worker` 时，“有排队任务但没有存活 Worker”是 failed；没有任务但 Worker 不在线是 warning。存活 Worker 的 global concurrency 不一致会阻止新实例加入，运维页仍会把历史漂移显示为 failed。运行中 Job 没有对应有效槽位、槽位心跳过期也会进入 attention queue。

## 评分不是玄学

运行治理中心的评分是用于定位风险的工程评分，不是业务价值评分。它的原则是：阻断项扣得重，warning 扣得轻，缺少可选组件在 `auto` 模式下不直接让系统不可用。

### 工作空间健康分

单个工作空间从 100 分开始：

| 项 | 扣分 |
| --- | --- |
| 阻断项 | 每个 `18` 分 |
| 风险项 | 每个 `4` 分 |

阻断项通常来自缺少关键产物、自动验证失败、产物契约失败或视觉检查失败。风险项通常来自数据质量 warning、验证 warning、视觉 warning、未知状态产物或仍在运行的队列。

工作空间健康状态按最严重结果收敛：

| 状态 | 含义 |
| --- | --- |
| `healthy` | 关键产物齐全，验证和视觉检查没有阻断 |
| `warning` | 可以打开，但有数据质量、视觉或队列风险 |
| `failed` | 存在明确阻断项，需要修复后再交付 |
| `unknown` | 产物或验证报告不足，还不能判断 |

运行治理中心还会给每个工作空间增加交付分层，用来避免历史失败项目污染主平台可用性判断：

| 分层 | 含义 |
| --- | --- |
| `可演示` | 关键产物和验证状态可用于展示 |
| `有风险` | 可以追踪，但存在 warning、未知验证或数据质量风险 |
| `待修复` | 存在验证、产物契约、视觉验收阻断，或仍有队列任务 |
| `归档候选` | 长时间未活跃且存在阻断项，应从主可用性口径中分离 |

### 治理总评分

运行总览有两层。

第一层是检查项摘要：失败项每个扣 `18` 分，warning 每个扣 `7` 分，unknown 每个扣 `3` 分。

第二层是健康画像，按权重聚合：

| 画像 | 因子 | 权重 |
| --- | --- | --- |
| 运行健康 | 核心服务 | 40 |
| 运行健康 | 工具链 | 20 |
| 运行健康 | 存储与表结构 | 25 |
| 运行健康 | 可观测性 | 15 |
| 项目健康 | 活跃交付分 | 45 |
| 项目健康 | 可演示占比 | 30 |
| 项目健康 | 修复压力 | 20 |
| 项目健康 | 历史归档 | 5 |
| 策略健康 | 策略目录 | 25 |
| 策略健康 | 股票池覆盖 | 30 |
| 策略健康 | 数据新鲜度 | 20 |
| 策略健康 | 回测准备度 | 15 |
| 策略健康 | 研究数据源 | 10 |

分数区间：

| 分数 | 状态 |
| --- | --- |
| `>= 85` | 正常 |
| `70 - 84` | 有风险 |
| `< 70` | 需要处理 |

## 日志怎么读

日志页会优先查 Loki。Loki 不可用时，会按降级配置读取本地日志文件，例如 `.next/dev/logs/next-development.log`、`tmp/runtime/*.log` 和评测队列日志。

建议按这个顺序筛选：

1. 先筛时间范围，缩到问题发生前后 10 到 30 分钟。
2. 再选择来源，例如 Next、market-data、eval 或 Loki。
3. 再按错误、警告、信息级别筛选，并搜索 `failed`、`validation`、`timeout` 等关键词。
4. 如果是一条 JSON 日志，先看 `source`、`level`、`message`，再看业务字段。

前端开发日志里会有很多编译成功信息，这些通常不是问题。真正值得关注的是错误栈、API 失败、SSE 断连、Agent runtime 返回的错误和验证失败摘要。

## 降级模式

运行治理中心要支持本地开发不完整组件的情况。

| 模式 | 适用场景 | 行为 |
| --- | --- | --- |
| `auto` | 日常开发 | 可选组件缺失给 warning，页面继续可用 |
| `strict` | CI、演示、生产巡检 | 必需组件缺失直接失败 |
| `offline` | 只看页面、只查本地文件 | 跳过市场数据后端和可观测组件探测 |

关键配置在 `.env`：

```env
QUANTPILOT_DEGRADATION_MODE=auto
QUANTPILOT_MARKET_API_REQUIRED=0
QUANTPILOT_OBSERVABILITY_REQUIRED=0
QUANTPILOT_REDIS_REQUIRED=0
```

如果用户反馈“页面打不开”，不要先把所有组件都重启一遍。先用运行治理中心或 `npm run doctor` 判断是哪一层缺失，再动对应组件。

## 常见排查路径

| 现象 | 先看 | 再看 |
| --- | --- | --- |
| 首页打不开 | 基础环境、Next dev 日志 | `lsof -i :3000`、`npm run dev` 输出 |
| 生成项目预览打不开 | 工作空间健康、预览端口 | 项目 `npm run build` 和预览进程 |
| 聊天页报 Agent 错误 | 生成链路观测、日志搜索 requestId | CLI 配置、模型返回、最大轮数 |
| 任务一直停在生成看板 | Data Agent 执行池的 Worker、队列和最久等待 | Worker 日志、dispatch lease、用户运行配额 |
| 策略平台加载慢 | 策略健康、market-data 日志 | 股票池分页、Redis、SQL 查询 |
| 补数任务不知道是否在跑 | 策略健康、补数 job 心跳 | `quant.market_data_ingestion_jobs` |
| Loki 没日志 | 可观测性检查、Alloy 状态 | 本地日志兜底是否可读 |
| 工作空间健康分低 | 产物列表和 next actions | validation、artifact contracts、visual validation |
| 主平台正常但项目健康很低 | 工作空间交付分层 | 先归档历史失败项目，再处理待修复项目 |

## 运行命令

```bash
npm run doctor
npm run doctor:full
```

启动可观测性组件：

```bash
npm run obs:up
npm run obs:logs
```

只想在没有外部组件时做本地排查：

```bash
QUANTPILOT_DEGRADATION_MODE=offline npm run doctor
```

## 运行治理中心未来还可以增强什么

短期最有价值的是：

- 把补数和评测队列继续收敛到与 generation 一致的可暂停、可恢复、可追踪 job 模型。
- 为关键 API 增加耗时和错误率指标，让 Grafana 不只看日志。
- 把工作空间健康快照写入 PostgreSQL，支持趋势对比。
- 将大日志、截图和长报告接对象存储，数据库只保存索引。
- 在日志页支持保存筛选条件，方便复盘同一类问题。
