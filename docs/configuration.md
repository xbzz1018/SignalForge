...
QUANTPILOT_QUERY_REWRITE_LLM_TIMEOUT_MS=12000
...
### 默认 Qwen + ModelPort DeepSeek + Memory
...
推荐完整拓扑（Qwen + ModelPort DeepSeek + Memory）的真实只读联调：

```bash
npm run check:integrations
```

该命令会产生真实模型 Token，并要求三条链路都存在；因此 Qwen-only、direct-only 或 Memory-disabled 环境不应把它的“缺少未启用组件”当成产品故障。此时按本篇对应章节分别验证实际启用的端点，再用 UI 完成一个 workspace 任务。
