...
这个目录保存项目的技术资料，也是完整文档索引。根目录 [README](../README.md) 负责 SignalForge 的定位和首次启动；长期配置、架构设计、教学材料、数据口径和排障经验在这里维护。许多专题沿用上游 QuantPilot 的内部标识与架构说明，不能据此将底层设计归为个人原创；当前可运行方式以根 README 和实际配置为准。
