...
东方财富财报 `NOTICE_DATE` 没有时区时按 Asia/Shanghai 解释后转换到 UTC；它只描述公告时间，不能证明本平台当时已经获得该内容。`REPORT_DATE` 仍是会计报告期。财报归档以公告时间和数据库首次观测时间的较晚者作为可用时间；缺公告时间保留原始内容，但禁止进入严格历史查询。具体规则见 [财报点时版本](data-dictionary.md#财报点时版本)。
