...
`quant.financial_report_versions` 由 `sqls/010-financial-report-versions.sql` 创建，通过管理员采集接口追加真实观测；数据库触发器禁止修改、删除或清空已有版本。

| 字段 | 口径 |
| --- | --- |
| `revision_id` | 每次内容变化的 UUID；同一证券/源/报告期连续相同内容复用版本，A → B → A 保留三次观测 |
| `symbol`、`provider`、`report_date` | 标准证券代码、来源和会计报告期，不把报告期当成公告日 |
| `notice_at` | 公告时间；东方财富无时区时间按 Asia/Shanghai 解释，缺失保留 null |
| `observed_at` | 取得同证券采集锁后由数据库时钟生成的首次观测时间，不能由请求回填 |
| `available_at` | max(公告时间, 观测时间)；缺公告时间时为 null，禁止参与历史查询 |
| `content_sha256`、`payload` | 规范化财报内容的校验和与完整 JSON；历史读取重新校验，损坏时停止返回 |

带 `as_of` 的财报/基本面查询同时限制可用时间、观测时间和报告期，按报告期选择截止时点内最新版本。`knowledge.data_version` 固化返回版本序列，`knowledge.vintages` 与报告/指标顺序对应，包含观测时间与校验和。无历史记录返回空样本和质量提示，数据库不可用返回 503；均不回退到最新接口或缓存。

该归档从首次真实采集开始积累，无法证明此前历史时点已知哪些数据；当前不覆盖历史行业成员、退市证券与复权事件，也未自动接入所有研究计划。
