...
当前 `api.py` 仍是兼容装配入口，`database.py` 已收敛为兼容门面。后续新增能力不再继续扩大这两个文件，优先落到目标目录，再由 router/service 或 legacy facade 显式挂载。
...
4. `database.py` 已收敛为兼容门面；后续新增 SQL 直接进入对应 repository，不再扩写门面。
...
- `services/*`、`routers/*` 和 `api.py` 不再直接依赖 `database.py`；已新增 `repositories/analytics.py`、`repositories/bars.py`、`repositories/coverage.py`、`repositories/foundation.py`、`repositories/ingestion.py`、`repositories/sector_flow.py`、`repositories/screener.py`、`repositories/universes.py`、`repositories/upserts.py` 和 `repositories/research.py`。`database.py` 已降为兼容门面，基础连接/转换函数迁入 `database_core.py`。
