...
后端长期按 Controller / Use Case / Repository / Provider Adapter 分层。当前 `api.py` 和 `database.py` 保留兼容入口，新增能力优先落到下面这些边界：
...
| `services/market-data/src/quantpilot_market_data/repositories/` | TimescaleDB/PostgreSQL 查询、事务、批量写入和分页 |
...
- 将 `services/market-data/src/quantpilot_market_data/database.py` 拆为 `repositories/bars.py`、`repositories/universes.py`、`repositories/factors.py`、`repositories/ingestion.py`、`repositories/sector_flow.py`、`repositories/foundation.py` 等 repository 文件，保留同一连接入口。
