...
- `src/lib/eval/`：评测用例、评测集、运行报告、队列、运行时选项和修复单。
- `strategies.ts`、`strategy-types.ts`、`strategy-catalog.ts`、`strategy-scan-repository.ts`、`strategy-readiness.ts`：股票池、ETF/指数池、策略模板、基础组件、数据质量扫描、补数任务、扫描报告和策略工作空间关联。
- `research-reports.ts`：投研日报自动化服务，管理观察池、日报运行、报告契约和推送 dry-run 记录。
...
- `src/lib/quant/strategies.ts` 已拆出 types、catalog、readiness 和 scan repository；后续继续拆 market API client、response mappers 和 dashboard service。
