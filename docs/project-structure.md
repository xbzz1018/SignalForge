...
| `.claude/skills/` | 受 registry/lock、版本与 SHA-256 完整性校验的历史 skills 源资产，仅供 MoAgent 编译器兼容读取；不参与运行时发现 |
...
- 生成链路已落 PostgreSQL durable job/outbox、claim/attempt/fencing 和 replan 封存；下一步把 completion 拆到独立 polling worker，并为评测、策略扫描复用同一调度合同。Redis 只承担可丢失的唤醒与进度缓存，锁和完成态权威始终留在 PostgreSQL。
