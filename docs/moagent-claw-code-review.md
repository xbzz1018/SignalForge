...
3. **阶段化工具 schema（历史实现）**：1.4 会在首次成功 workspace write 前隐藏 `submit_result`，并在读取收敛后移除 read schema。该方案的权限收敛目标保留，但 schema 增删会破坏 prefix cache，已在 1.8 被“物理 run 内固定 schema、执行器动态拒绝”取代。
...
1. **受信 PhaseGraph**：平台准备状态在接触模型前确定 `deterministic_standard`、`model_custom`、`model_repair` 或 `model_data_preparation` lane，模型不能选择 lane。custom 与 repair 都被限制为最多 3 个模型回合；data-preparation 保留最多 8 回合的受限能力面。
2. **standard 零模型 renderer**：平台预检通过后，Provider-compatible 的可信计划依次发出 `apply_dashboard_spec` 和 `submit_result`，不发起网络/模型请求并严格报告零 Token。两步仍经过同一个 Run Engine、operation ledger、workspace fencing、事件、写后提交和 terminal gate；失败不会伪装完成。
3. **物理 run 内固定工具 schema**：PhaseGraph 在启动时确定最小工具集合，Run Engine 后续不再增删或重排 Provider 可见定义。首次写入、读取预算和 failure scope 仍由执行器动态拒绝，因此固定 schema 不代表权限扩大，同时减少无意义的 prompt-prefix cache break。
4. **ProgressOracle 实时收敛**：新增 Provider-neutral、确定性、可序列化的 reducer/class，并接入 Run Engine；通用默认阈值为 2 回合，三回合 custom/repair 收紧为 1 回合。首次无进展先给模型一次软纠偏和定向重读机会，连续第二次无进展或重复 observation 后才由执行器硬拒绝 read，避免 workspace conflict、失效 pointer/anchor 无法恢复。重复工具活动、无 digest/同内容写入和 workspace A→B→A 回访不算进展。Oracle 不判定完成，其状态尚未写入 durable checkpoint；运行时没有回合内验证器时也不会虚构 failed-check 进展。
5. **版本身份**：当前 framework identity 为 `moagent:1.8.0`。durable Generation Coordinator 并未随本次版本升级完成；贯穿全部平台 writer 的阶段 lease/fencing、崩溃 dispatcher 和自动接管仍是下一阶段 P0。
...
早期固定审计从 Claw Code 吸收了请求边界、流建立前失败恢复和事件身份；扩展复核又把信息增益、缓存前缀、语义编辑、任务隔离与独立验收纳入比较。当前已实现 PhaseGraph、standard 零模型确定性 renderer、custom/repair 三回合硬预算、物理 run 内固定工具 schema、ProgressOracle 实时收敛、Observation Ledger、Prompt Prefix Ledger、Trusted ContextCapsule、Semantic Edit、DashboardSpec、原子 Skill Capsules、Mission Graph、EvidenceVerifier、generation active slot、typed writer durable pre-image journal 与真实 E2E 证据门。ProgressOracle 的 durable checkpoint、贯穿全部平台 writer 的 Generation Coordinator、客户端事件补发和受限只读并行仍未完成。typed workspace writer 可自动回滚，但 external/uncertain 操作仍必须阻断调和。
...
| 执行图与阶段工具面 | MoAgent 增强 | PhaseGraph 在模型前选择 lane；standard 以零模型 Token 运行 compiler + submit，custom/repair 最多 3 个模型回合；每个物理 run 的最小 schema 固定，权限在执行器内动态收紧 |
| 进展判定 | MoAgent 增强 | ProgressOracle 已接入实时 Run Engine；custom/repair 一轮无净进展先软纠偏，连续两轮或重复 observation 后才进入执行器 read gate，真实内容前进后解除；不判定完成，durable checkpoint 尚未实现 |
...
### P0：下一阶段是 Durable Generation Coordinator
...
PostgreSQL active slot 已阻止同项目的并发非终态 Mission，并以 generationId 绑定 planning、prefetch、candidate、validation、preview 与 acceptance；candidate 的 CAS 验证认领和手工验证状态门也阻断了重复验收入口。下一阶段必须实现 durable Generation Coordinator：让全部平台 writer 共享 monotonic fencing token、可接管阶段 lease、持久 dispatch intent 与崩溃后的 dispatcher。外层 generation queue 当前仍有文件投影和分段锁区间，因此不能把已有 Mission active slot 或 MoAgent typed-writer fence 描述为完整 coordinator。
...
Trusted ContextCapsule 与 Semantic Edit 已在 1.7 移出路线图；PhaseGraph、固定 Provider schema 和 ProgressOracle 实时收敛已在 1.8 移出路线图。剩余工作是：

1. **Node-scoped executor policy**：system 已只保留 Skill manifest，运行时按 phase/signal/hash/budget 注入原子 capsule；PhaseGraph 也已在物理 run 启动时固定 standard/custom/repair/data-preparation 的最小 schema。下一步让每个 Mission node 直接编译数据 prerequisite 与执行器权限，同时继续保持该 run 的 Provider schema 不变，而不是恢复回合级 schema 增删。
...
本轮增强的验收覆盖：PhaseGraph 的确定性 lane 路由、安全不变量、standard 零模型路径以及 custom/repair 三回合预算；可信工具计划的严格零 Token usage、固定顺序、失败停止和未知工具拒绝；Run Engine 在回合间保持固定 Provider 工具定义并在执行器内拒绝未授权调用；ProgressOracle 对新增可信事实、workspace 内容前进、失败项减少、重复 observation、无 digest/同内容写入与 A→B→A 回访做 reducer、序列化和实时接线回归，并验证连续停滞只触发收敛而不伪造完成。既有验收还覆盖超限 instruction/requestId 与 Provider 请求在网络前被拒绝；DeepSeek identity/usage 算术漂移失败关闭，缺失 usage 保守计入 input/cache miss；Context Manager 保持工具簇原子；同参数 workspace 读取在未写入时只执行一次；DashboardSpec 对数据 identity、排名、交易明细、趋势/量能/风险失败关闭；durable sink 在工具前失败时副作用为零；多文件 journal 故障恢复、本机死亡锁接管和 PostgreSQL 竞争/权威时钟/takeover 契约。ProgressOracle durable checkpoint、目标共享卷验收、durable Generation Coordinator 和生产迁移演练仍是发布前置条件。
