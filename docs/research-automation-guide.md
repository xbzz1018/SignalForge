...
投研日报模块把“自选池、数据采样、报告契约、推送记录”串成一条可验证链路。它参考多数据源日报项目的优点，但不把 QuantPilot 变成外部 API Key 拼装脚本：真实行情仍从本地 market-data 读取，报告先保存为结构化对象，再由页面、推送和后续 LLM 摘要共同使用。
...
| 路由 | 方法 | 责任 |
| --- | --- | --- |
| `/api/research/reports` | `GET` | 返回日报中心首页数据、观察池、最新报告、运行历史和推送记录 |
