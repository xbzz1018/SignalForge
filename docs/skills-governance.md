...
1. 在 `http://localhost:3000/skills` 选择需要维护的核心 skill。
2. 在线编辑 `SKILL.md`，或者拖拽 `.zip`、`.tgz`、`.tar.gz` 上传完整 skill 包。
3. 填写 semver 版本号、发布摘要和变更点。
4. 点击发布后，平台会更新 registry、changelog、tgz 包和 lock。
5. 运行 `npm run check:skills`，确认源码 hash、包 hash 和版本记录一致。
...
`/skills` 工作台的上传发布会限制到已登记核心 skill，并拒绝路径穿越、软链接、硬链接和异常文件数量。发布或打包失败时会回滚 registry、changelog、lock 和压缩包；上传包失败时会回滚源码目录。
