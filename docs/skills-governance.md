...
4. 创建项目时，编译器适配旧路径和旧工具名，把验证后的内容安装为 `<workspace>/.moagent/skills/` 参考镜像；创建服务只以安装成功或抛错作为结果，不把 receipt 注入 Agent。
5. 每次 Agent 执行重新按第 1～3 步从仓库受信输入只读编译 system context；当前不从 workspace `.moagent/skills/` 或 `.claude/skills/` 发现能力。
