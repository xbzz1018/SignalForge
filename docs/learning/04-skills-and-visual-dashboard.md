...
Skills 是 Agent 的本地能力包。仓库根目录 `.claude/skills/` 是保留历史命名、受 registry/lock、版本与 SHA-256 完整性校验的当前编译源，目前没有密码学签名；加载顺序是 source 优先、受校验 tgz fallback。项目初始化会把校验后的参考镜像配置到生成工作空间 `.moagent/skills/`；Agent 执行阶段仍从仓库输入只读编译本次上下文，不从 workspace 镜像发现能力，也不重新安装镜像。
...
| 文件 | 用途 |
| --- | --- |
| `.claude/skills.registry.json` | 仓库兼容源的 skills 注册表 |
| `.claude/skills.lock.json` | 仓库兼容源的当前锁定版本 |
| `.claude/skills.changelog.json` | 仓库兼容源的版本变更记录 |
| `.claude/skill-packages/*.tgz` | 仓库兼容源中受 lock 与 SHA-256 完整性校验的打包产物 |
