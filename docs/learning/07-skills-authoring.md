...
本教程中的 `.claude/**` 均指仓库根目录保留、受 registry/lock、版本与 SHA-256 完整性校验的 Skill 源兼容区，也是当前 Agent 的 source-first 编译输入，目前没有密码学签名。项目初始化会把校验并适配后的参考镜像配置到 `<workspace>/.moagent/skills/`；Agent 执行阶段仍从仓库输入只读编译上下文，不从 workspace 镜像发现能力。
...
上面的路径只用于仓库内创作、完整性校验和发布；Agent 运行时不会从 workspace `.claude/skills/` 发现能力。
...
1. 打开 `.claude/skills/dashboard-visualization/SKILL.md`。
