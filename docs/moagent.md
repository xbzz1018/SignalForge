...
MoAgent 是 QuantPilot 自研的 Agent 框架。当前框架身份为 `moagent:1.8.0`。它以进程内 TypeScript 模块运行；需要模型的 lane 默认通过 ModelPort 使用 Qwen，也可通过 ModelPort 使用 Anthropic 协议上游 DeepSeek，另保留 DeepSeek 官方 OpenAI-compatible 直连 profile；可信标准看板 lane 则执行零模型 Token 的确定性工具计划。这些路径都不依赖外部 Agent SDK、CLI 子进程或供应商 session。
...
| `QUANTPILOT_QUERY_REWRITE_LLM_TIMEOUT_MS` | `12000` | Query Rewrite 单次语义调用硬超时，允许 500–15000ms |
