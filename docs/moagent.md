...
MoAgent 是 QuantPilot 自研的 Agent 框架。当前框架身份为 `moagent:1.7.0`。它以进程内 TypeScript 模块运行，直接调用 DeepSeek 官方 OpenAI-compatible API，不依赖外部 Agent SDK、CLI 子进程或供应商 session。
...
5. 平台先对 run plan、final data、sources 和 data quality 做语义就绪与 DashboardSpec 预检；空 `{}`、标的覆盖不足、模板不一致、数据 run identity 漂移或已知编译前置条件不满足都会留在 data-preparation/custom surface，不让模型先调用一个必败工具。普通生成只注入带 digest 的短 DashboardSpec receipt；明确的布局、样式、组件或“不要卡片”等定制才在 Provider 前生成有界源码合同并进入 semantic-edit surface。两条 surface 都移除泛目录读取、行情 API、无附件图片工具和 `write_file`/`edit_file`/`apply_patch` 三套冗余 mutation schema。每轮请求前，Context Manager 只接受框架为具体工具注册的浅层 canonical receipt projector；第三方工具即使自带同名字段也不能把路径或错误码提升为可信系统状态。没有 projector 的工具只生成框架自有 outcome tombstone：保留 call/tool/effect/status 与参数、结果摘要哈希，不吸收第三方原文或其声称的 target。被覆盖历史的最近精确 tombstone 有固定上限，更旧事实进入带计数的 hash-chain rollup；详细 read/artifact receipt 仍按重取成本淘汰，写入立即使旧 read receipt 失效。这样副作用簇可以被原子替换，而不会因永久保留大段 raw tool output 挤爆上下文。产品历史只继承真实用户消息与标记为 final 的助手结论。
6. Run Engine 请求 DeepSeek；Provider 对 response ID、choice、tool-call identity、终止顺序和 Token 算术执行严格状态校验。`total != input + output`、cache hit/miss 不闭合或 reasoning 超过 output 均作为协议错误；Provider 完全缺少 usage 时，运行时按本轮 prepared request 做保守 input/cache-miss 估算并标记 `usageSource=estimated`，不能以零绕过累计预算。
...
平台预取 generation 不再使用一个固定的宽工具面。standard surface 只暴露 `apply_dashboard_spec` 与 `submit_result`：平台预检已经证明该 renderer 与数据前置条件可执行，不把已知失败交给模型试错；custom surface 只暴露 `query_json`、`query_text_file`、`semantic_edit`、`submit_result`。两者都不发送平台 inspector、legacy whole-file mutation、行情 API 或无附件图片工具。repair 没有静态宽权限：平台把 failed check ID 编译成 `app/page.tsx`、`app/globals.css`、明确 final/evidence 目录或唯一 API route 的精确 allowlist，未知失败项不再退化为 `app/**`。失败报告缺失、陈旧或没有明确失败项时，模型调用前即失败关闭。
...
每条 AgentRun 持久化 `frameworkVersion=moagent:1.7.0` 与 `buildRevision`。CI/deployment 优先使用显式 revision；本地 dirty checkout 会把 tracked diff 以及所有 untracked path、类型、长度和内容 SHA-256 纳入有界 fingerprint，无法完整读取时显式标记 `dirty.unavailable`，不会冒充干净 HEAD。数据库 migration 为旧记录写入 `legacy:pre-1.7`，避免把历史运行误认成当前框架证据。

确定性 contract benchmark 只验证平台合同，不能证明模型 Agent 能力。schema v4 发布证据必须由真实 `/act -> Mission -> EvidenceVerifier -> accepted receipt` 链路产生，并同时匹配当前 framework/build/git、DeepSeek provider/model、request identity、完整 E2E case 集、时间顺序和 receipt hash；`unversioned:*` 与 `dirty.unavailable` 构建不能作为 E2E 发布证据。每个 case 保存逐物理 run 的有界安全摘要，accepted receipt 必须绑定 lineage 中状态为 `candidate_complete` 的 source run，候选来源必须是 `moagent_submit_result`，且 ledger 至少证明一次成功 workspace write 与一次 `submit_result`；平台恢复或安全模板候选不能冒充 MoAgent 能力成绩。Token 证明按 run 与聚合双重要求 `input > 0`、`cached + cacheMiss = input`、`input + output = total`；字段缺失或把 cache miss 伪装为零都不能通过效率门。
...
| `MOAGENT_CONTEXT_CAPSULE_MAX_BYTES` | `8192` | 单次 Trusted ContextCapsule 上限；超限先按优先级淘汰，仍放不下则保留 canonical 历史并跳过压缩，不让压缩失败影响工具正确性 |
| `MOAGENT_MAX_RUN_INPUT_TOKENS` | `160000` | 累计输入 Token 上限；优先使用严格校验的 Provider usage，缺失时按 prepared request 保守估算 |
| `MOAGENT_MAX_RUN_CACHE_MISS_INPUT_TOKENS` | `120000` | 累计非缓存输入 Token 上限；Provider 缺 usage 时把本轮保守 input 计为 cache miss，不能以零绕过 |
