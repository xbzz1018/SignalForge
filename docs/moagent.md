...
| Skills | `src/lib/agent/skills/` | registry/version/SHA-256 完整性校验、source-first/package-fallback 加载、按 capability 选择和有界上下文编译；项目初始化只配置 workspace 参考镜像 |
...
3. 项目初始化阶段把通过 registry、版本与 SHA-256 完整性校验的 Skill 参考镜像配置到 `.moagent/skills`。当前 Agent 执行源仍是仓库 `.claude/skills`（source 优先，受校验 tgz 仅在 source 缺失时 fallback）；执行阶段只读编译本次 capability 所需的有界 system context，不从 workspace 镜像发现能力，也不在取得 lease 前改写它。
...
5. 平台预取模式先在进程内生成一次只读 `initial_dashboard_contract`，把计划、final/evidence 摘要、页面合同和源码 outline 随首轮任务注入；该模式只编译可视化 skill，并移除泛目录读取、原始 JSON 读取、行情 API 和无附件图片工具。每轮请求前，Context Manager 再按内部输入预算压缩历史：保留 system、最新 user 和最近工具簇；普通旧 reasoning 优先删除，旧工具输出只做确定性摘要，工具调用簇只能整体保留或整体淘汰。产品历史只继承真实用户消息与标记为 final 的助手结论，自动验证、repair 流水线和 Agent 工具旁白不会进入下一次 run。
...
| `query_json` | 单次批量查询最多 16 个 RFC 6901 JSON Pointer；大数组只返回头部与较新的尾部样本，并显式记录省略信息、shape 与 SHA-256 |
| `query_text_file` | 单次批量查询最多 16 个组件、函数或 CSS selector 字面锚点，公平返回有界行窗，替代完整源码和连续区间扫描 |
...
平台预取 generation 的默认工具面只有定向写入、`inspect_dashboard_contract`、`query_json`、`query_text_file` 和 `submit_result`；无附件时不注册图片工具，已预取时不注册行情 API。repair profile 额外开放 `data_file/final/**` 和 `evidence/**`，用于验证报告明确要求的数据契约修复。任何 profile 都不能写 `.quantpilot/**`，final/evidence 和超过工具输出预算的有效大 JSON 也不能再通过 raw reader 顺序扫描。
...
- 新 Skill 必须进入 registry/lock、通过完整性校验，并设置 system context 字符预算。
...
| `MOAGENT_SKILL_CONTEXT_CHARS` | `16000` | 非预取流程的 Skill system context 字符预算 |
| `MOAGENT_PREFETCHED_SKILL_CONTEXT_CHARS` | `6000` | 平台预取 generation/repair 的可视化相关 Skill 字符预算 |
