...
MoAgent 是 QuantPilot 自研的 Agent 框架。当前框架身份为 `moagent:1.12.0`。它以进程内 TypeScript 模块运行；需要模型的 lane 默认通过 ModelPort 使用 Qwen，也可通过 ModelPort 使用 Anthropic 协议上游 DeepSeek，另保留 DeepSeek 官方 OpenAI-compatible 直连 profile；可信标准看板 lane 则执行零模型 Token 的确定性工具计划。这些路径都不依赖外部 Agent SDK、CLI 子进程或供应商 session。
...
1. 外层编排先取得项目级 generation lease，再在共享文件系统资源锁内生成只读 run plan、预取真实量化数据和 evidence；同一 project 的另一 Web/worker 进程不能并发覆盖这些输入产物。
...
`.quantpilot/generation-queue.json` 从 1.12 起只由 PostgreSQL job/outbox 重新物化，不再参与 claim、取消或完成判定。读取投影时会审计过期 dispatch：只有 generation lease、AgentRun lease 和 Mission verification lease 都不能证明仍有活 worker，才把 job、UserRequest 和未完成 Mission 关闭为 `interrupted/failed + replan_required`；这避免外层心跳偶发延迟误杀仍在运行的细粒度执行。当前接收请求的 Web 进程仍负责启动已 claim 的 completion，尚未提供独立轮询 worker 自动创建新 attempt；进程崩溃后系统确定性封存旧 attempt，用户重试会从持久化状态重新规划，而不是伪造原会话续跑。
...
这不是对任意共享存储的无条件跨主机保证。生产挂载必须让所有实例看到同一 canonical root，并支持跨客户端原子 `mkdir`、同文件系统原子 `rename` 和所需 fsync 语义；当前自动化覆盖单 Node 进程、两个 Prisma client、本机故障注入与启动回滚，目标 NFS/CSI/分布式卷仍必须另做多进程、多主机断电验收。generation job/outbox、generation lease、AgentRun/workspace 与 Mission verification 已有分层 lease/fencing，`.quantpilot/generation-queue.json` 只是可再生投影。进程崩溃后旧 attempt 会被封存且旧写入被拒绝；系统仍不会自动恢复旧 Provider 私有会话，也尚未由独立 worker 自动创建 replan attempt，更不对外承诺跨数据库与任意存储的 exactly-once。
...
每条 AgentRun 持久化 `frameworkVersion=moagent:1.12.0` 与 `buildRevision`。CI/deployment 优先使用显式 revision；本地 dirty checkout 会把 tracked diff 以及所有 untracked path、类型、长度和内容 SHA-256 纳入有界 fingerprint，无法完整读取时显式标记 `dirty.unavailable`，不会冒充干净 HEAD。数据库 migration 曾为 1.7 之前的旧记录写入 `legacy:pre-1.7`；这是历史迁移标记，不会把旧运行重标为 1.12 证据。
