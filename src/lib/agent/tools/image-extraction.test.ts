...
async function makeWorkspace(): Promise<string> {
  const workspace = await fs.mkdtemp(path.join(os.tmpdir(), 'moagent-image-tool-'));
...
describe('MoAgent image extraction tool', () => {
  it('validates attachment metadata and preserves the manual-confirmation contract', async () => {
    ...
    await fs.writeFile(path.join(workspace, '.quantpilot', 'attachments.json'), JSON.stringify({
      attachments: [{
        id: 'portfolio-1',
        name: 'portfolio.png',
  ...
  it('returns the compatible no-attachments status', async () => {
    const workspace = await makeWorkspace();
    await fs.writeFile(
      path.join(workspace, '.quantpilot', 'attachments.json'),
