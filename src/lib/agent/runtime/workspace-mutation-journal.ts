...
export const PI_AGENT_WORKSPACE_MUTATION_JOURNAL_DIRECTORY =
  '.pi-mutation-journal';

const JOURNAL_SCHEMA_VERSION = 'pi-agent-workspace-mutation-journal-v1';
...
export interface PiAgentWorkspaceMutationJournal {
  workspaceRoot: string;
  transactionDirectory: string;
  manifest: WorkspaceMutationJournalManifest;
}

export class PiAgentWorkspaceMutationRecoveryConflictError extends Error {
  constructor(readonly target: string, message: string) {
    super(message);
    this.name = 'PiAgentWorkspaceMutationRecoveryConflictError';
  }
}
...
function assertIdentifier(value: unknown, label: string): asserts value is string {
  if (typeof value !== 'string' || value.length === 0 || value.length > 512 || /[\r\n\0]/.test(value)) {
    throw new Error(`Invalid PI Agent mutation journal ${label}.`);
  }
}
...
function assertRelativePath(value: unknown, label: string): asserts value is string {
  ...
  if (
    ...
    throw new Error(`Unsafe PI Agent mutation journal ${label}.`);
  }
}
...
function parseManifest(value: unknown): WorkspaceMutationJournalManifest {
  if (!isRecord(value) || value.schemaVersion !== JOURNAL_SCHEMA_VERSION) {
    throw new Error('Unsupported or corrupt PI Agent mutation journal manifest.');
  }
  ...
  if (!['prepared', 'committing', 'applied', 'rolled_back'].includes(String(value.state))) {
    throw new Error('Invalid PI Agent mutation journal state.');
  }
  if (typeof value.createdAt !== 'string' || !Number.isFinite(Date.parse(value.createdAt))) {
    throw new Error('Invalid PI Agent mutation journal createdAt.');
  }
  if (!Array.isArray(value.files) || value.files.length === 0 || value.files.length > 8) {
    throw new Error('Invalid PI Agent mutation journal file set.');
  }
  const targets = new Set<string>();
  const files = value.files.map((candidate, index): WorkspaceMutationJournalFile => {
    if (!isRecord(candidate)) throw new Error(`Invalid PI Agent mutation journal file ${index}.`);
    ...
    if (typeof candidate.existedBefore !== 'boolean') {
      throw new Error(`Invalid PI Agent mutation journal files[${index}].existedBefore.`);
    }
    if (!Number.isSafeInteger(candidate.mode) || Number(candidate.mode) < 0 || Number(candidate.mode) > 0o777) {
      throw new Error(`Invalid PI Agent mutation journal files[${index}].mode.`);
    }
    ...
    if (
      ...
      throw new Error(`Inconsistent PI Agent mutation journal file ${index}.`);
    }
    if (targets.has(candidate.target)) {
      throw new Error(`Duplicate PI Agent mutation journal target ${candidate.target}.`);
    }
...
async function canonicalWorkspaceRoot(workspaceRoot: string): Promise<string> {
  ...
  if (!stat.isDirectory()) throw new Error('PI Agent mutation journal workspace must be a directory.');
  return root;
}
...
async function assertNoSymlinkComponents(base: string, candidate: string): Promise<void> {
  ...
  if (relative.startsWith('..') || path.isAbsolute(relative)) {
    throw new Error('PI Agent mutation journal path escaped its root.');
  }
  let current = base;
  for (const segment of relative.split(path.sep).filter(Boolean)) {
    ...
    try {
      ...
      if (stat.isSymbolicLink()) {
        throw new PiAgentWorkspaceMutationRecoveryConflictError(
...
async function ensureJournalRoot(workspaceRoot: string): Promise<string> {
  const journalRoot = path.join(workspaceRoot, PI_AGENT_WORKSPACE_MUTATION_JOURNAL_DIRECTORY);
  try {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink()) {
    throw new Error('PI Agent mutation journal root must be a framework-owned directory.');
  }
...
async function readManifest(transactionDirectory: string): Promise<WorkspaceMutationJournalManifest> {
  ...
  if (!stat.isFile() || stat.isSymbolicLink()) {
    throw new Error('PI Agent mutation journal manifest must be a regular file.');
  }
...
export async function preparePiAgentWorkspaceMutationJournal(options: {
  workspaceRoot: string;
  runId: string;
  operationId: string;
  files: readonly PrepareWorkspaceMutationJournalFile[];
}): Promise<PiAgentWorkspaceMutationJournal> {
  ...
  if (options.files.length === 0 || options.files.length > 8) {
    throw new Error('A PI Agent mutation journal must contain between 1 and 8 files.');
  }
  ...
  try {
    ...
    for (const [index, input] of options.files.entries()) {
      ...
      if (input.existedBefore !== (input.beforeSha256 !== null)) {
        throw new Error(`Inconsistent PI Agent mutation journal input ${input.target}.`);
      }
      if (sha256(input.content) !== input.afterSha256) {
        throw new Error(`PI Agent mutation journal after hash mismatch for ${input.target}.`);
      }
...
export async function setPiAgentWorkspaceMutationJournalState(
  journal: PiAgentWorkspaceMutationJournal,
...
export async function commitPiAgentWorkspaceMutationJournal(
  journal: PiAgentWorkspaceMutationJournal,
): Promise<void> {
  ...
  for (const file of journal.manifest.files) {
    ...
    if (!stagedStat.isFile() || stagedStat.isSymbolicLink() || sha256(await fs.readFile(staged)) !== file.afterSha256) {
      throw new Error(`PI Agent mutation journal staged content is corrupt for ${file.target}.`);
    }
    await fs.rename(staged, target);
    await syncDirectory(path.dirname(target));
  }
  await setPiAgentWorkspaceMutationJournalState(journal, 'applied');
}
 ...
 */
export async function rollbackPiAgentWorkspaceMutationJournal(
  journal: PiAgentWorkspaceMutationJournal,
): Promise<void> {
  ...
  for (const file of journal.manifest.files) {
    ...
    if (current !== file.beforeSha256 && current !== file.afterSha256) {
      throw new PiAgentWorkspaceMutationRecoveryConflictError(
        file.target,
        `Refusing to recover ${file.target} because it was modified after the interrupted PI Agent write.`,
      );
    }
    if (file.existedBefore) {
      ...
      if (sha256(backup) !== file.beforeSha256) {
        throw new Error(`PI Agent mutation journal backup is corrupt for ${file.target}.`);
      }
    }
  ...
  for (const { file, current } of states) {
    ...
    if (!file.existedBefore) {
      ...
      const restorePath = path.join(
        path.dirname(target),
        `.${path.basename(target)}.pi-restore-${randomUUID()}.tmp`,
      );
      const backupPath = pathInside(journal.transactionDirectory, file.backup!);
      await assertNoSymlinkComponents(journal.transactionDirectory, backupPath);
      const backup = await fs.readFile(backupPath);
...
export async function cleanupPiAgentWorkspaceMutationJournal(
  journal: PiAgentWorkspaceMutationJournal,
): Promise<void> {
...
export async function listPiAgentWorkspaceMutationJournals(
  workspaceRoot: string,
): Promise<PiAgentWorkspaceMutationJournal[]> {
  const root = await canonicalWorkspaceRoot(workspaceRoot);
  const journalRoot = path.join(root, PI_AGENT_WORKSPACE_MUTATION_JOURNAL_DIRECTORY);
  let entries;
  try {
  ...
  const journals: PiAgentWorkspaceMutationJournal[] = [];
  for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
    if (!entry.isDirectory() || !SHA256_PATTERN.test(entry.name)) {
      throw new Error('Unexpected entry in the PI Agent mutation journal directory.');
    }
    ...
    if (transactionName(manifest.operationId) !== entry.name) {
      throw new Error('PI Agent mutation journal operation identity mismatch.');
    }
