import type { QuantCapabilityId } from './capabilities';
...
const VARIANTS_BY_TEMPLATE_ID: Record<string, QuantVisualizationTemplateVariant[]> = {
  ...
  'technical-timing': [
    ...
    },
    {
      ...
      layout: 'trigger-strip-over-kline-detail',
      density: 'balanced',
      match: { keywords: ['突破', '支撑', '压力', '买点', '卖点', '触发', '止损'], maxSymbols: 1 },
      requiredComponents: ['关键价位带', '突破/失效条件', 'K 线主图', '量能确认', '风险提示'],
      optionalComponents: ['观察清单', '交易计划草案'],
      firstViewport: ['关键价位带', 'K 线主图', '触发条件'],
  ...
  'fundamental-research': [
    {
      ...
      layout: 'quality-ledger-plus-trend-table',
      density: 'balanced',
  ...
  'stock-selection': [
    {
      ...
      requiredComponents: ['标的覆盖摘要', '多标的指标矩阵', '收益对比主图', '回撤/波动主图', '排序依据', '数据口径'],
  ...
  'strategy-research': [
    ...
    },
    {
      ...
      requiredComponents: ['信号规则表', 'K 线信号叠加', '参数表', '风险过滤', '待回测清单'],
      optionalComponents: ['初步回测', '参数敏感性'],
      firstViewport: ['信号规则表', 'K 线信号叠加', '参数表'],
