/**
 * Read-only PostgreSQL catalog checks for the durable PI Agent runtime schema.
 *
...
export interface PiAgentCatalogColumn {
  tableName: string;
  columnName: string;
  dataType: string;
  nullable: boolean;
...
export interface PiAgentCatalogCheckConstraint {
  tableName: string;
...
export interface PiAgentCatalogIndex {
  tableName: string;
  indexName: string;
  columns: string[];
  unique: boolean;
...
export interface PiAgentCatalogForeignKey {
  tableName: string;
  constraintName: string;
  columns: string[];
...
export interface PiAgentSchemaCatalog {
  columns: readonly PiAgentCatalogColumn[];
  indexes: readonly PiAgentCatalogIndex[];
  foreignKeys: readonly PiAgentCatalogForeignKey[];
  checkConstraints: readonly PiAgentCatalogCheckConstraint[];
}

export type PiAgentSchemaIssueCode =
  | 'MISSING_TABLE'
  | 'MISSING_COLUMN'
...
export interface PiAgentSchemaIssue {
  code: PiAgentSchemaIssueCode;
  objectName: string;
  expected: string;
  actual?: string;
}

export interface PiAgentSchemaReadinessReport {
  ready: boolean;
  contractVersion: typeof PI_AGENT_SCHEMA_CONTRACT_VERSION;
  issues: PiAgentSchemaIssue[];
}
...
export const PI_AGENT_SCHEMA_EXPECTATIONS = {
...
function issueSortKey(issue: PiAgentSchemaIssue): string {
  return `${issue.objectName}:${issue.code}:${issue.expected}:${issue.actual ?? ''}`;
}

export async function readPiAgentSchemaCatalog(
  client: PiAgentSchemaQueryClient
): Promise<PiAgentSchemaCatalog> {
  ...
  const columns = await client.$queryRawUnsafe<PiAgentCatalogColumn[]>(
    CATALOG_COLUMNS_SQL
  );
  const indexes = await client.$queryRawUnsafe<PiAgentCatalogIndex[]>(
    CATALOG_INDEXES_SQL
  );
  const foreignKeys = await client.$queryRawUnsafe<PiAgentCatalogForeignKey[]>(
    CATALOG_FOREIGN_KEYS_SQL
  );
  const checkConstraints = await client.$queryRawUnsafe<PiAgentCatalogCheckConstraint[]>(
    CATALOG_CHECK_CONSTRAINTS_SQL
  );
  return { columns, indexes, foreignKeys, checkConstraints };
}

export function evaluatePiAgentSchemaCatalog(
  catalog: PiAgentSchemaCatalog
): PiAgentSchemaReadinessReport {
  const issues: PiAgentSchemaIssue[] = [];
  ...
  return {
    ...
    contractVersion: PI_AGENT_SCHEMA_CONTRACT_VERSION,
    issues,
  };
}

export async function inspectPiAgentSchemaReadiness(
  client: PiAgentSchemaQueryClient
): Promise<PiAgentSchemaReadinessReport> {
  return evaluatePiAgentSchemaCatalog(await readPiAgentSchemaCatalog(client));
}

export class PiAgentSchemaNotReadyError extends Error {
  readonly code = 'PI_AGENT_SCHEMA_NOT_READY' as const;
  readonly contractVersion = PI_AGENT_SCHEMA_CONTRACT_VERSION;
  readonly issues: PiAgentSchemaIssue[];

  constructor(report: PiAgentSchemaReadinessReport) {
    super(
      `PI Agent database schema ${report.contractVersion} is not ready (${report.issues.length} issue${report.issues.length === 1 ? '' : 's'}). Apply the checked-in Prisma migrations before accepting agent runs.`
    );
    this.name = 'PiAgentSchemaNotReadyError';
    this.issues = report.issues;
  }
}

export async function assertPiAgentSchemaReady(
  client: PiAgentSchemaQueryClient
): Promise<PiAgentSchemaReadinessReport> {
  const report = await inspectPiAgentSchemaReadiness(client);
  if (!report.ready) throw new PiAgentSchemaNotReadyError(report);
  return report;
}
