...
import { createPiAgentOperationId } from '@/lib/agent/core/operation-id';
import {
...
import type { PiAgentEvent, PiAgentToolCall } from '@/lib/agent/types';
import {
  createPiAgentDurableRunSession,
  PiAgentToolReplayBlockedError,
  type PiAgentRunStoreScheduler,
} from './pi-agent-run-store';
...
class ManualScheduler implements PiAgentRunStoreScheduler {
...
function eventBase(sequence: number) {
  return {
    runId: 'pi_agent_durable-test',
    sequence,
    eventId: `pi_agent_durable-test:${sequence}`,
    timestamp: START.getTime(),
  };
}

function runStarted(sequence = 1): PiAgentEvent {
  return {
    ...eventBase(sequence),
    type: 'run_started',
    provider: 'deepseek',
...
function toolBase(sequence: number, toolCall = call()) {
  return {
    ...
    turn: 1,
    toolCall,
    operationId: createPiAgentOperationId('pi_agent_durable-test', 1, toolCall),
    effect: 'workspace_write' as const,
    idempotency: 'reconcile_required' as const,
  };
}

describe('PiAgentDurableRunSession', () => {
  it('atomically marks an approval wait and resumes without preparing the tool ledger', async () => {
    ...
    const session = await createPiAgentDurableRunSession({
  ...
  it('persists a canonical ProgressOracle snapshot at the model-turn boundary', async () => {
    ...
    expect(readPiAgentProgressOracleCheckpoint(checkpoint!)).toMatchObject({
      version: 1,
      turnsObserved: 1,
      consecutiveNoProgressTurns: 1,
      lastWorkspaceFingerprint: 'b'.repeat(64),
  ...
  it('prepares/completes the ledger before events, skips deltas, and persists only safe checkpoints', async () => {
    ...
    const invalidFailure: PiAgentEvent = {
  ...
  it('treats an existing operation ledger entry as a hard execution gate', async () => {
    ...
    await expect(session.record(started)).rejects.toBeInstanceOf(PiAgentToolReplayBlockedError);
    expect(session.failure).toBeInstanceOf(PiAgentToolReplayBlockedError);
  ...
  it('interrupts under the current fence and reports heartbeat failures through onFatal', async () => {
    ...
    const failedSession = await createPiAgentDurableRunSession({
