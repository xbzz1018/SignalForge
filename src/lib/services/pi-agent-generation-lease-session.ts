...
import {
  ...
  type PiAgentGenerationLeaseClaim,
  type PiAgentGenerationLeaseFence,
  type PiAgentGenerationStage,
...
export interface PiAgentGenerationLeaseSessionOptions {
  leaseOwner?: string;
  leaseTtlMs?: number;
 ...
 */
export class PiAgentGenerationLeaseSession {
  ...
  private constructor(
    readonly claim: PiAgentGenerationLeaseClaim,
    options: {
      leaseTtlMs: number;
      heartbeatIntervalMs: number;
      heartbeatEnabled: boolean;
    ...
    if (options.heartbeatEnabled) {
      this.timer = globalThis.setInterval(() => {
        ...
        void this.serialize(async () => {
          await heartbeatPiAgentGenerationLease({
            fence: this.fence,
  ...
  static async claimProject(
    ...
    options: PiAgentGenerationLeaseSessionOptions = {}
  ): Promise<PiAgentGenerationLeaseSession> {
    const leaseTtlMs = configuredPositiveInteger(
      'PI_AGENT_GENERATION_LEASE_TTL_MS',
      options.leaseTtlMs,
      DEFAULT_LEASE_TTL_MS
    );
    const heartbeatIntervalMs = configuredPositiveInteger(
      'PI_AGENT_GENERATION_HEARTBEAT_INTERVAL_MS',
      options.heartbeatIntervalMs,
    ...
    if (heartbeatEnabled && heartbeatIntervalMs >= leaseTtlMs) {
      throw new Error(
        'PI_AGENT_GENERATION_HEARTBEAT_INTERVAL_MS must be smaller than the lease TTL.'
      );
    }
    ...
    const claim = await claimPiAgentGenerationLease({
    ...
    return new PiAgentGenerationLeaseSession(claim, {
      leaseTtlMs,
      heartbeatIntervalMs,
      heartbeatEnabled,
    });
  ...
  get fence(): PiAgentGenerationLeaseFence {
    return {
  ...
  async release(): Promise<void> {
    ...
    await this.serialize(async () => {
      this.assertHealthy();
      await releasePiAgentGenerationLease({ fence: this.fence });
    }, true);
    this.settled = true;
  }
  ...
  async dispose(): Promise<void> {
    ...
    this.stopHeartbeat();
    try {
      await this.serialize(() => releasePiAgentGenerationLease({ fence: this.fence }), false);
    } finally {
      this.settled = true;
    }
  }
...
export async function withPiAgentGenerationLease<T>(input: {
  projectId: string;
  operationId?: string;
  requestId?: string | null;
  stage: PiAgentGenerationStage;
  options?: PiAgentGenerationLeaseSessionOptions;
  task: (session: PiAgentGenerationLeaseSession) => Promise<T>;
}): Promise<T> {
  const session = await PiAgentGenerationLeaseSession.claimProject(input, input.options);
  try {
