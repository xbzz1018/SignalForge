...
export const PI_AGENT_GENERATION_STAGES = [
  'planning_data_prefetch',
  'agent_execution',
  'manual_validation',
] as const;

export type PiAgentGenerationStage = (typeof PI_AGENT_GENERATION_STAGES)[number];

export interface PiAgentGenerationLeaseFence {
  projectId: string;
  operationId: string;
  leaseOwner: string;
  fencingToken: number;
...
export interface PiAgentGenerationLeaseClaim extends PiAgentGenerationLeaseFence {
...
export class PiAgentGenerationLeaseError extends Error {
  constructor(
    ...
    super(message);
    this.name = 'PiAgentGenerationLeaseError';
  }
}

function assertIdentifier(value: string, label: string): void {
  if (!value || Buffer.byteLength(value, 'utf8') > MAX_IDENTIFIER_BYTES || /[\r\n]/.test(value)) {
    throw new PiAgentGenerationLeaseError(
...
function assertLeaseInput(input: {
  ...
  if (!PI_AGENT_GENERATION_STAGES.includes(input.stage)) {
    throw new PiAgentGenerationLeaseError(
      'GENERATION_LEASE_INVALID',
...
function assertFence(
  ...
  fence: PiAgentGenerationLeaseFence,
  now: Date
): void {
  if (
    lease.status !== 'held' ||
...
export async function claimPiAgentGenerationLease(input: {
  projectId: string;
  operationId: string;
  requestId?: string | null;
  stage: PiAgentGenerationStage;
  leaseOwner: string;
  leaseTtlMs: number;
}): Promise<PiAgentGenerationLeaseClaim> {
...
export async function heartbeatPiAgentGenerationLease(input: {
  fence: PiAgentGenerationLeaseFence;
  leaseTtlMs: number;
...
export async function releasePiAgentGenerationLease(input: {
  fence: PiAgentGenerationLeaseFence;
}): Promise<void> {
  return prisma.$transaction(async (tx) => {
    const now = await databaseNow(tx);
