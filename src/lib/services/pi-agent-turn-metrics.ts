...
function safeSum(values: readonly number[], label: string): number {
  ...
  if (!Number.isSafeInteger(result) || result < 0) {
    throw new Error(`PI Agent ${label} aggregation exceeded the safe integer range.`);
  }
  return result;
}
...
export async function collectPiAgentTurnMetrics(params: {
  projectId: string;
  requestId: string;
  ...
  if (!request) {
    throw new Error('PI Agent turn metrics cannot find the root user request.');
  }
