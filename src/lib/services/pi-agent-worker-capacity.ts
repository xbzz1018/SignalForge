...
export interface PiAgentWorkerSlotFence {
  poolKey: string;
  slotNumber: number;
  activeJobId: string;
  leaseOwner: string;
...
export interface PiAgentWorkerSlotClaim extends PiAgentWorkerSlotFence {
  leaseExpiresAt: string;
}

export class PiAgentWorkerCapacityError extends Error {
  constructor(readonly code: string, message: string) {
    super(message);
    this.name = 'PiAgentWorkerCapacityError';
  }
}
...
function assertInput(input: {
  ...
  if (!POOL_PATTERN.test(input.poolKey)) {
    throw new PiAgentWorkerCapacityError(
      'WORKER_POOL_INVALID',
...
export async function claimPiAgentWorkerSlot(input: {
  poolKey: string;
  capacity: number;
  activeJobId: string;
  leaseOwner: string;
  leaseTtlMs: number;
}): Promise<PiAgentWorkerSlotClaim | null> {
  assertInput(input);
  try {
...
export async function heartbeatPiAgentWorkerSlot(input: {
  fence: PiAgentWorkerSlotFence;
  leaseTtlMs: number;
...
export async function releasePiAgentWorkerSlot(
  fence: PiAgentWorkerSlotFence,
): Promise<void> {
  await mutateOwnedSlot({ fence, release: true });
}

export class PiAgentWorkerCapacitySession {
  private timer: ReturnType<typeof setInterval> | null = null;
  private fatalError: Error | null = null;

  private constructor(
    readonly claim: PiAgentWorkerSlotClaim,
    private readonly leaseTtlMs: number,
    heartbeatIntervalMs: number,
  ) {
    this.timer = setInterval(() => {
      if (this.fatalError) return;
      void heartbeatPiAgentWorkerSlot({
        fence: this.claim,
  ...
  static async tryClaim(input: {
    ...
    leaseTtlMs: number;
    heartbeatIntervalMs: number;
  }): Promise<PiAgentWorkerCapacitySession | null> {
    ...
    const claim = await claimPiAgentWorkerSlot({
    ...
    return claim
      ? new PiAgentWorkerCapacitySession(
          claim,
          input.leaseTtlMs,
  ...
  async release(): Promise<void> {
    ...
    this.timer = null;
    this.assertHealthy();
    await releasePiAgentWorkerSlot(this.claim);
  }
}
