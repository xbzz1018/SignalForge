...
const TERMINAL_STATUSES = new Set<PiAgentGenerationDispatchStatus>([
  "completed",
  "failed",
  "cancelled",
  "interrupted",
...
export const PI_AGENT_GENERATION_DISPATCH_STATUSES = [
  "pending",
  "running",
  "retry_wait",
  "completed",
...
export type PiAgentGenerationDispatchStatus =
  (typeof PI_AGENT_GENERATION_DISPATCH_STATUSES)[number];

export type PiAgentGenerationDispatchTerminalStatus = Extract<
  PiAgentGenerationDispatchStatus,
  "completed" | "failed" | "cancelled" | "interrupted"
>;

export interface PiAgentGenerationDispatchFence {
  jobId: string;
  projectId: string;
  requestId: string;
  leaseOwner: string;
...
export interface PiAgentGenerationDispatchClaim extends PiAgentGenerationDispatchFence {
  attemptCount: number;
...
export class PiAgentGenerationDispatchError extends Error {
  constructor(
...
function assertIdentifier(value: string, label: string): void {
  if (
    ...
    throw new PiAgentGenerationDispatchError(
...
function configuredEnvelopeMaxBytes(): number {
  const configured = Number.parseInt(
    process.env.PI_AGENT_DISPATCH_ENVELOPE_MAX_BYTES ?? "",
    10,
  );
  ...
  if (
    ...
    throw new PiAgentGenerationDispatchError(
      "GENERATION_DISPATCH_INVALID",
      "PI_AGENT_DISPATCH_ENVELOPE_MAX_BYTES must be between 1024 and 4194304.",
    );
  }
  return configured;
}
...
function configuredPendingOrphanGraceMs(): number {
  const configured = Number.parseInt(
    process.env.PI_AGENT_DISPATCH_PENDING_ORPHAN_GRACE_MS ?? "",
    10,
  );
  ...
  if (
    ...
    throw new PiAgentGenerationDispatchError(
      "GENERATION_DISPATCH_INVALID",
      "PI_AGENT_DISPATCH_PENDING_ORPHAN_GRACE_MS must be between 1000 and 86400000.",
    );
  }
  return configured;
}
...
function retryDelayMs(attemptCount: number): number {
  const configured = Number.parseInt(
    process.env.PI_AGENT_DISPATCH_RETRY_BASE_DELAY_MS ?? "",
    10,
  );
  ...
  if (!Number.isSafeInteger(base) || base < 100 || base > 60 * 60 * 1_000) {
    throw new PiAgentGenerationDispatchError(
      "GENERATION_DISPATCH_INVALID",
      "PI_AGENT_DISPATCH_RETRY_BASE_DELAY_MS must be between 100 and 3600000.",
    );
  }
...
export async function enqueuePiAgentGenerationJob(input: {
  projectId: string;
  requestId: string;
  instruction: string;
...
export async function claimPiAgentGenerationJob(input: {
  projectId: string;
  requestId: string;
  leaseOwner: string;
  leaseTtlMs: number;
}): Promise<PiAgentGenerationDispatchClaim> {
  assertIdentifier(input.projectId, "projectId");
  assertIdentifier(input.requestId, "requestId");
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const now = await databaseNow(tx);
    if (
      TERMINAL_STATUSES.has(current.status as PiAgentGenerationDispatchStatus)
    ) {
      throw new PiAgentGenerationDispatchError(
...
export async function heartbeatPiAgentGenerationJob(input: {
  fence: PiAgentGenerationDispatchFence;
  leaseTtlMs: number;
...
export async function finishPiAgentGenerationJob(input: {
  projectId: string;
  requestId: string;
  status: Exclude<PiAgentGenerationDispatchTerminalStatus, "cancelled">;
...
export async function cancelPiAgentGenerationJob(input: {
  projectId: string;
  requestId: string;
  reason?: string | null;
}): Promise<AgentGenerationJob | null> {
...
export async function reconcileExpiredPiAgentGenerationJobs(
  input: {
    projectId?: string;
    limit?: number;
  } = {},
...
export async function listPiAgentGenerationJobs(
  projectId: string,
  limit = 50,
): Promise<AgentGenerationJob[]> {
...
export async function getPiAgentGenerationJob(
  projectId: string,
  requestId: string,
): Promise<AgentGenerationJob | null> {
  assertIdentifier(projectId, "projectId");
...
export async function listClaimablePiAgentGenerationJobs(
  limit = 20,
): Promise<AgentGenerationJob[]> {
...
export async function listPendingPiAgentGenerationOutboxEvents(
  projectId: string,
  limit = 200,
...
export async function markPiAgentGenerationOutboxEventsPublished(
