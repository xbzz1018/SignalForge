...
export class PiAgentToolApprovalStoreError extends Error {
  constructor(
    readonly code:
      | 'INVALID_APPROVAL'
      | 'APPROVAL_NOT_FOUND'
    ...
    super(message);
    this.name = 'PiAgentToolApprovalStoreError';
  }
}

function assertApprovalId(value: string): string {
  if (!APPROVAL_ID_PATTERN.test(value)) {
    throw new PiAgentToolApprovalStoreError(
      'INVALID_APPROVAL',
...
async function createApproval(request: PiAgentToolApprovalRequest): Promise<void> {
  assertApprovalId(request.approvalId);
  const projectedInput = publicInput(
    request.publicInput,
    'PI Agent approval public input',
  );
...
}): PiAgentToolApprovalResolution | null {
  ...
  if (status === 'edited' && record.editedInput) {
    return {
      ...
      editedInput: publicInput(
        record.editedInput,
        'PI Agent stored edited approval input',
      ),
      resolvedBy: record.resolvedByActorId,
    };
  }
...
export function createPrismaPiAgentToolApprovalHandler(
  pollIntervalMs = DEFAULT_POLL_INTERVAL_MS,
): PiAgentToolApprovalHandler {
  if (!Number.isSafeInteger(pollIntervalMs) || pollIntervalMs <= 0 || pollIntervalMs > 10_000) {
    throw new Error('PI Agent approval poll interval must be between 1 and 10000 ms.');
  }
  return async (
    request: PiAgentToolApprovalRequest,
    context: { signal: AbortSignal },
  ): Promise<PiAgentToolApprovalResolution> => {
...
export async function resolvePiAgentToolApproval(input: {
  projectId: string;
  approvalId: string;
  actorId: string;
  actorUserId?: string;
  ...
  const editedInput = input.decision === 'edit'
    ? publicInput(input.editedInput, 'PI Agent approval edited input')
    : undefined;
...
export async function listPiAgentToolApprovals(input: {
  projectId: string;
  runId?: string;
  status?: ApprovalStatus;
  limit?: number;
  ...
  return records
    .map((record) => ({
      ...
      decision: record.decision as PiAgentToolApprovalDecision | null,
    }))
    .sort((left, right) =>
...
export async function readPiAgentRunTimeline(input: {
  projectId: string;
  runId: string;
