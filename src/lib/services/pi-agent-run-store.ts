...
import {
  ...
  PI_AGENT_CHECKPOINT_STATE_VERSION,
...
export interface PiAgentRunStoreScheduler {
  setInterval(callback: () => void, intervalMs: number): unknown;
  clearInterval(handle: unknown): void;
}

export interface PiAgentDurableRunInput extends AgentRunProvenance {
  runId: string;
...
export interface CreatePiAgentDurableRunSessionOptions {
  repository: AgentRuntimeRepository;
  run: PiAgentDurableRunInput;
  leaseOwner?: string;
  leaseTtlMs?: number;
  heartbeatIntervalMs?: number;
  heartbeatEnabled?: boolean;
  clock?: () => Date;
  scheduler?: PiAgentRunStoreScheduler;
  onFatal?: (error: unknown) => Awaitable<void>;
  onTerminalRun?: (run: AgentRunRecord) => Awaitable<void>;
}

export type CreatePrismaPiAgentDurableRunSessionOptions = Omit<
  CreatePiAgentDurableRunSessionOptions,
  'repository'
>;

export interface InterruptPiAgentDurableRunOptions {
  code?: string;
  turnCount?: number;
...
export class PiAgentToolReplayBlockedError extends Error {
  constructor(
    readonly operationId: string,
    readonly executionStatus: string
  ) {
    super(`PI Agent operation ${operationId} already exists in state ${executionStatus}; execution is blocked.`);
    this.name = 'PiAgentToolReplayBlockedError';
  }
}

const defaultScheduler: PiAgentRunStoreScheduler = {
...
function asError(error: unknown): Error {
  return error instanceof Error ? error : new Error('PI Agent durable store failed.');
}
...
function normalizeUsage(usage: PiAgentTokenUsage): AgentTokenUsage {
  return {
...
function durableStatus(status: PiAgentRunStatus): TerminalAgentRunStatus {
  switch (status) {
    case 'completed':
...
function completionError(status: PiAgentRunStatus, projected: RuntimeJsonObject): {
  code: string;
  message: string;
} | undefined {
  ...
  return { code, message: `PI Agent run ended with ${code}.` };
}
...
function resultStateHashes(event: Extract<PiAgentEvent, { type: 'tool_completed' }>): {
  preStateHash?: string;
...
/**
 * Product-layer durable sink for one physical PI Agent run.
 *
 ...
 */
export class PiAgentDurableRunSession {
  private readonly repository: AgentRuntimeRepository;
  private readonly leaseOwner: string;
  private readonly leaseTtlMs: number;
  private readonly heartbeatIntervalMs: number;
  private readonly clock: () => Date;
  private readonly scheduler: PiAgentRunStoreScheduler;
  ...
  private constructor(
    options: CreatePiAgentDurableRunSessionOptions,
    run: AgentRunRecord,
    leaseOwner: string,
    leaseTtlMs: number,
    heartbeatIntervalMs: number
  ...
  static async create(
    options: CreatePiAgentDurableRunSessionOptions
  ): Promise<PiAgentDurableRunSession> {
    ...
    return new PiAgentDurableRunSession(
      { ...options, clock },
      run,
      leaseOwner,
      leaseTtlMs,
  ...
  async record(event: PiAgentEvent): Promise<void> {
    if (this.closed) throw new Error('PI Agent durable run session is closed.');
    if (event.runId !== this.currentRun.id) {
      throw new Error('PI Agent event belongs to a different durable run.');
    }
  ...
  async interrupt(options: InterruptPiAgentDurableRunOptions = {}): Promise<void> {
    this.stopHeartbeat();
    return this.serialize(async () => {
      if (this.terminal) return;
      ...
      this.currentRun = await this.repository.completeRun({
        ...
        error: {
          code,
          message: `PI Agent run was interrupted with ${code}.`,
        },
      });
  ...
  private fence(now: Date): AgentWriteFence {
    if (
      ...
      throw new Error('PI Agent durable run no longer owns its lease.');
    }
    return {
  ...
  private async prepareTool(
    event: Extract<PiAgentEvent, { type: 'tool_started' }>,
    ...
    if (!prepared.created) {
      throw new PiAgentToolReplayBlockedError(operationId, prepared.execution.status);
    }
  }

  private async completeTool(
    event: Extract<PiAgentEvent, { type: 'tool_completed' | 'tool_failed' }>,
    projection: RuntimeJsonObject
  ): Promise<void> {
  ...
  private async saveCheckpoint(
    ...
    const stateHash = hashPiAgentCheckpointPublicState(publicState);
    const saved = await this.repository.saveCheckpoint({
      ...
      publicState,
      stateHash,
      stateVersion: PI_AGENT_CHECKPOINT_STATE_VERSION,
    });
    this.currentRun = saved.run;
  }

  private async completeRun(
    event: Extract<PiAgentEvent, { type: 'run_finished' }>,
    projection: RuntimeJsonObject
  ): Promise<void> {
  ...
  private async notifyTerminalRun(): Promise<void> {
    ...
    try {
      ...
      console.error('[PI Agent] Terminal run notification failed:', error);
    }
  }

  private observeCounters(event: PiAgentEvent): void {
...
export async function createPiAgentDurableRunSession(
  options: CreatePiAgentDurableRunSessionOptions
): Promise<PiAgentDurableRunSession> {
  return PiAgentDurableRunSession.create(options);
}

export async function createPrismaPiAgentDurableRunSession(
  options: CreatePrismaPiAgentDurableRunSessionOptions
): Promise<PiAgentDurableRunSession> {
  const clock = options.clock ?? (() => new Date());
  return createPiAgentDurableRunSession({
    ...options,
    clock,
