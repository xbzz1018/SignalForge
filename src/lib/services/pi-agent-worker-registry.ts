...
export interface PiAgentWorkerInstanceClaim {
  id: string;
  poolKey: string;
  hostname: string;
  processId: number;
...
export class PiAgentWorkerRegistryError extends Error {
  constructor(readonly code: string, message: string) {
    super(message);
    this.name = 'PiAgentWorkerRegistryError';
  }
}
...
function assertInput(input: {
  ...
  if (!POOL_PATTERN.test(input.poolKey)) {
    throw new PiAgentWorkerRegistryError(
...
export async function registerPiAgentWorkerInstance(input: {
  id?: string;
  poolKey?: string;
  hostname?: string;
  processId?: number;
  ...
  leaseTtlMs: number;
  retentionMs?: number;
}): Promise<PiAgentWorkerInstanceClaim> {
  const normalized = {
...
export async function heartbeatPiAgentWorkerInstance(input: {
  claim: PiAgentWorkerInstanceClaim;
  leaseTtlMs: number;
...
export async function stopPiAgentWorkerInstance(
  claim: PiAgentWorkerInstanceClaim,
): Promise<void> {
...
export class PiAgentWorkerRegistrySession {
  private timer: ReturnType<typeof setInterval> | null = null;
  private fatalError: Error | null = null;

  private constructor(
    readonly claim: PiAgentWorkerInstanceClaim,
    private readonly leaseTtlMs: number,
    heartbeatIntervalMs: number,
  ) {
    this.timer = setInterval(() => {
      if (this.fatalError) return;
      void heartbeatPiAgentWorkerInstance({
        claim: this.claim,
  ...
  static async start(input: {
    ...
    leaseTtlMs: number;
    heartbeatIntervalMs: number;
  }): Promise<PiAgentWorkerRegistrySession> {
    ...
    const claim = await registerPiAgentWorkerInstance(input);
    return new PiAgentWorkerRegistrySession(
      claim,
      input.leaseTtlMs,
  ...
  async stop(): Promise<void> {
    if (this.timer) clearInterval(this.timer);
    this.timer = null;
    await stopPiAgentWorkerInstance(this.claim);
  }
}
