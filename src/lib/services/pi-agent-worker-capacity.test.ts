...
import {
  claimPiAgentWorkerSlot,
  heartbeatPiAgentWorkerSlot,
  releasePiAgentWorkerSlot,
} from './pi-agent-worker-capacity';
...
describe('PI Agent global Worker capacity', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('claims one database-backed slot with a monotonic fencing token', async () => {
    ...
    const claim = await claimPiAgentWorkerSlot(claimInput());
  ...
  it('returns null when the shared pool has no free slot', async () => {
    ...
    await expect(claimPiAgentWorkerSlot(claimInput())).resolves.toBeNull();
  ...
  it('heartbeats and releases only the exact fenced owner', async () => {
    ...
    await expect(heartbeatPiAgentWorkerSlot({
      fence,
      leaseTtlMs: 120_000,
    })).resolves.toEqual({
      leaseExpiresAt: '2026-07-23T05:02:00.000Z',
    });
    await expect(releasePiAgentWorkerSlot(fence)).resolves.toBeUndefined();
    expect(mocks.transactionClient.agentWorkerSlot.updateMany)
