...
import type { PiAgentRunResult } from '@/lib/agent/types';
import { hashPiAgentWorkspace } from '@/lib/services/pi-agent-provenance';
...
export async function capturePiAgentCandidate(input: {
  ...
  const workspace = await hashPiAgentWorkspace(input.workspaceRoot);
 ...
 */
export async function refreshPiAgentCandidateWorkspace(input: {
  workspaceRoot: string;
...
export async function candidateFromPiAgentRun(input: {
  workspaceRoot: string;
  requestId?: string;
  ...
  const summary = typeof data?.summary === 'string'
    ? data.summary
    : terminal?.content ?? 'PI Agent candidate submitted';
