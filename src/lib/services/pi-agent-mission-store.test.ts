...
function candidate(): PiAgentCandidateSubmission {
  return {
    schemaVersion: 1,
    source: 'pi_agent_submit_result',
...
function evidenceDecision(input: {
  candidateVersion?: number;
  payloadOverrides?: Partial<PiAgentEvidenceDecision['payload']>;
} = {}): PiAgentEvidenceDecision {
  ...
  return {
    ...
    payload,
    receiptHash: `sha256:${hashPiAgentProvenance(payload)}`,
  };
}
...
function expectMissionError(error: unknown, code: string): void {
  expect(error).toBeInstanceOf(PiAgentMissionStateError);
...
describe('PI Agent Mission candidate durability', () => {
  it('seals submit-result as candidate_complete without completing the Mission or UserRequest', async () => {
    ...
    const result = await recordPiAgentMissionCandidate({
      missionId: MISSION_ID,
      projectId: PROJECT_ID,
      requestId: REQUEST_ID,
      candidate: candidate(),
...
describe('PI Agent Mission verification claim', () => {
  it('claims candidate_complete exactly once before validation can start', async () => {
    ...
    await expect(beginPiAgentMissionVerification({
      missionId: MISSION_ID,
      projectId: PROJECT_ID,
      requestId: REQUEST_ID,
...
describe('PI Agent Mission acceptance fence', () => {
  it('rejects a late evidence write from the worker fenced by takeover', async () => {
    ...
    const error = await recordPiAgentMissionEvidenceDecision({
      missionId: MISSION_ID,
      projectId: PROJECT_ID,
      requestId: REQUEST_ID,
  ...
  it('commits the acceptance receipt, Mission completion, and UserRequest completion in one transaction', async () => {
    ...
    const result = await recordPiAgentMissionEvidenceDecision({
      missionId: MISSION_ID,
      projectId: PROJECT_ID,
      requestId: REQUEST_ID,
      verificationFence: VERIFICATION_FENCE,
